...
- **Per-profile timeouts** (fields in `ProfileConfig`, loaded from `profiles.yaml`):
  - `shell_timeout_seconds` (default `30`) — defined in `ProfileConfig` but currently not applied; `_HtmlSafeShellBackend` (which enforced this timeout) was removed in favour of `LocalShellBackend`; the Grok HTML-encoding workaround is now handled via `register_harness_profile` in `atelier/main.py`.
