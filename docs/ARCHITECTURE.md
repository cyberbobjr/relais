...
- **Extracted modules**:
  ...
  - `atelier/prompts.py` — system-prompt constants and builders: `LONG_TERM_MEMORY_PROMPT`, `SELF_DIAGNOSIS_PROMPT`, `DIAGNOSTIC_MARKER`, `DIAGNOSTIC_AWARENESS_PROMPT`, `build_project_context_prompt`, `_build_execution_context`, `_enrich_system_prompt`; extracted from `agent_executor.py`.
