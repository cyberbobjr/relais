...
- Assemble le prompt système avec `assemble_system_prompt()` (`atelier/soul_assembler.py`), qui retourne un `AssemblyResult(prompt, issues, is_degraded)`. Si `is_degraded=True` (au moins une couche de prompt manquante ou illisible), un WARNING est loggué avec la liste des issues, et le prompt dégradé est quand même utilisé (fail-soft).
...
- Exécute `AgentExecutor` — retourne `AgentResult(reply_text, messages_raw, tool_call_count, tool_error_count, subagent_traces)`. `subagent_traces` est un tuple de `SubagentTrace` (un par sous-agent ayant utilisé des outils) construit depuis les callbacks `SubagentMessageCapture` ; tuple vide quand aucun sous-agent n'est invoqué.
- Publie :
...
- **Modules extraits** :
  ...
  - `atelier/subagents_resolver.py` — fonctions pures de résolution des tokens `tools:` et `skills:` (`_load_tools_from_import`, `_resolve_skill_token`, `_add_tool`, …) ; utilise `common.pattern_matcher.matches` pour le filtrage fnmatch ; extraites de `atelier/subagents.py`.
  - `atelier/display_config.py` — `DisplayConfig` (dataclass frozen) chargée depuis `atelier.yaml` (section `display:`) via `load_display_config()`, remplace l'ancien `progress_config.py`. La validation est par champ : chaque clé invalide émet un WARNING et retombe sur sa valeur par défaut (les autres champs restent appliqués) via `_validate_bool()` et `_validate_int()`.
  ...
  - `atelier/diagnostic_trace.py` — formatage de la trace diagnostique post-erreur : `_DIAGNOSTIC_MAX_CHARS`, `format_diagnostic_trace`, `_render_diagnostic_trace` ; extraits de `agent_executor.py`. Réexportés depuis `agent_executor.py` pour compatibilité descendante. Importe `DiagnosticTrace` depuis `atelier/errors.py` (dataclass structurant les compteurs et les détails d'erreur d'un tour avorté : `messages_count`, `tool_count`, `tool_errors`, `last_tool`, `last_error`, `tool_error_details`).
