    ...
    async def _stream(
        ...
        if capture is not None and ns_to_name:
            ...
            for ns_id, subagent_name in ns_to_name.items():
                sa_messages, sa_tool_count, sa_tool_errors = capture.get_subagent_data(ns_id)
                traces_list.append(SubagentTrace(
                    subagent_name=subagent_name,
                    skill_names=self._subagent_skill_map.get(subagent_name, []),
                    tool_call_count=sa_tool_count,
                    tool_error_count=sa_tool_errors,
                    messages_raw=serialize_messages(sa_messages),
                ))
