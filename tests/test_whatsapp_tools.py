"""Unit tests for channels.whatsapp.tools — the 3 BaseTools."""
...
from channels.whatsapp import core as core_module
...
class TestWhatsAppInstallTool:
    ...
    def test_tool_metadata(self) -> None:
        from channels.whatsapp.tools import whatsapp_install
    ...
    def test_happy_path(self) -> None:
        from channels.whatsapp.tools import whatsapp_install
        from channels.whatsapp.core import InstallResult, ApiKeyResult, StepResult

        with patch("channels.whatsapp.tools.core") as mock_core:
...
class TestWhatsAppConfigureTool:
    ...
    def test_tool_metadata(self) -> None:
        from channels.whatsapp.tools import whatsapp_configure
    ...
    def test_health_action(self) -> None:
        ...
        with patch("channels.whatsapp.tools.asyncio") as mock_asyncio:
...
class TestWhatsAppUninstallTool:
    ...
    def test_tool_metadata(self) -> None:
        from channels.whatsapp.tools import whatsapp_uninstall

        assert whatsapp_uninstall.name == "whatsapp_uninstall"

    def test_basic_uninstall(self) -> None:
        from channels.whatsapp.tools import whatsapp_uninstall
        from channels.whatsapp.core import StepResult

        with patch("channels.whatsapp.tools.core") as mock_core, \
             patch("channels.whatsapp.tools.asyncio") as mock_asyncio:
            mock_core.resolve_relais_home.return_value = Path("/tmp/.relais")
            mock_core.resolve_project_root.return_value = Path("/tmp")
...
class TestToolInstances:
    ...
    def test_all_tools_exported(self) -> None:
        from channels.whatsapp.tools import whatsapp_install, whatsapp_configure, whatsapp_uninstall
